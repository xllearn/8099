...
- 日采集公告约9000～10000条。
- 内部验证阶段的历史事实是：正式进入AI分析链路的任务通常每天10份以内，使用者约为测试人员2～3人、业务人员3～5人。
- 客户化包装完成态按**日均300～500份分析任务、活动期峰值约1000份/日**进行容量设计，以支持公司外部客户使用、批量分析和用户反馈修订。
- 上述300～500份/日与1000份/日是**目标容量和简历模拟口径**，不是当前真实生产业务量；对外表述时必须说明测试条件或容量规划依据。
...
|---|---:|---|---|
| 内部验证阶段 | 通常不超过10份/日 | 历史业务事实 | 单个主Worker即可运行，但仍需异步化与失败恢复 |
| 客户化常态 | 日均300～500份 | 目标容量/模拟口径 | 解析、生成、质检、评测队列隔离，Worker可水平扩容 |
| 活动期峰值 | 约1000份/日 | 峰值容量/模拟口径 | 需要限流、配额、优先级队列、缓存复用和积压恢复 |

不能用“日采集9000～10000条公告”直接替代“日分析任务量”。前者是进入资料库的原始公告规模，后者是客户发起并实际进入Evidence、生成、质检与交付链路的任务规模。
...
- 附件解析、报告生成、质量检查、用户修订、导出和DeepEval使用独立队列，避免低优先级评测阻塞客户报告。
- Worker根据队列积压、队列等待P95和模型供应商并发配额进行水平扩容或收缩。
- 客户请求通过账号配额、令牌桶限流和任务优先级控制突发流量；超过容量时进入可观测队列，而不是丢失任务或无限同步等待。
- 附件较多时可并行解析，并通过内容哈希复用相同公告和附件的解析资产。
...
- 客户化目标容量：日均300～500份分析任务；
- 活动期峰值容量：约1000份分析任务/日；
- 峰值提交速率：50份任务/小时；
- 最大稳定并发运行数：20个正式分析任务；
- 队列等待P95：60秒以内；
...
使用这些数字时必须同时说明测试集、数据分层、Worker数量、模型并发配额、附件规模和Judge模型，否则不要描述为真实成果。
...
> 将原本仅供内部业务人员使用的公告报告生成工具，升级为面向企业客户的共享行业知识分析平台；基于公开公告库和公司审核知识库，通过RAG自动召回历史项目、政策规则、价格采购量数据和风险案例，为报告生成、跨项目对比和用户反馈修订提供证据支持，并按日均300～500份、峰值约1000份/日的目标容量设计异步任务与弹性扩缩容体系。
...
| 客户化后的日均任务量 | 内部阶段每天10份以内是历史事实；客户化完成态按日均300～500份、峰值约1000份/日设计，均为目标容量/模拟口径 | 问题日志第19题、本文件第2与8节 |
...
- `docs/career/medical-notice-analyzer-question-response-log.md`：用户问题和应对记录，当前下一题编号为20。
...
- 将客户化包装完成态调整为日均300～500份分析任务、活动期峰值约1000份/日的目标容量。
- 明确容量数字属于模拟和规划口径，不是当前真实生产业务量。
- 增加峰值提交速率、稳定并发、队列等待和积压恢复等系统容量指标。
- 将Celery叙事从低频内部异步任务升级为队列隔离、弹性Worker、账号限流和模型并发配额控制。
- 同步问题日志第19题，下一题编号更新为20。
