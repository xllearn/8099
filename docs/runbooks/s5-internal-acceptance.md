...
| S5 worktree | `C:\Users\admin\.config\superpowers\worktrees\medical-notice-analyzer-8099-prod\s5-internal-acceptance` | 2026-07-17 基线检查时 clean；本次仅变更获批的四份受控文档，提交和推送后须复核 clean |
...
本 S5 文档提交将用户指定的当前权威计划以精确字节纳入同路径受控文件：

`docs/superpowers/plans/2026-07-09-8099-optimization-implementation-plan.md`

其 SHA-256 必须保持为
`BFDB9E67B6138ABA50864C8A8BD56CA37AF6C0C7DA0571FCD1CE8795258D49BA`。
该 hash、文档提交及远端同名 feature branch 一致性全部验证后，“权威计划尚未进入受控版本”缺口关闭；该文档提交不代表生产部署，也不改变 `88309d57...` 生产代码基线。
...
7. `/health` 非 HTTP 200 或非 ok、任一 UI 非 HTTP 200，或出现新增应用严重错误时，同一项目负责人立即停止新任务、记录异常并处置。
8. 同一项目负责人将检查写入仓库外受控运维日志，记录时间、部署 SHA、health/UI、worker、restart/OOM、错误计数、结论和处置；异常时保留日志和状态，不清理运行数据。
...
接口：`POST /analysis/runs/{run_id}/recover`。这是线上写操作，本手册不授权执行。只有用户另行明确授予 `RECOVER_AUTHORIZED` 后，才可由同一项目负责人在获批维护窗口执行。
...
- 事后由同一项目负责人在仓库外受控运维日志记录批准消息引用、执行时间、相关 ID、结果、结论和处置。
...
- [ ] 已确认本项目采用单人负责模式，同一项目负责人已核对当次明确授权范围及全部适用门禁。
...
本项目为单人负责，全部职责由同一项目负责人承担。不记录真实姓名，不划分发布、验收、前端、Dify 配置、运行值守、业务验收或记录维护等独立人员角色，也不设置职责分离或角色兼任门槛。

同一项目负责人负责：

- 核对 clean SHA、制品、备份、dry-run、部署身份和回退边界；
- 执行 M2/M3、固定 3/10、Word、状态和 UI/API 门禁复核；
- 核对前端变化范围及已发布 Dify workflow/prompt/model 的非敏感证据；
- 执行健康、日志、容器、历史、OOM/restart 观察和证据记录；
- 判断正式可交付和人工复核结论；
- 门禁通过后维护优化记录和证据索引。

单人负责不授权跳过门禁，不授权未经明确批准的 LIVE、部署、恢复或回退，不允许把 history/diagnostics 当作事实证据，也不允许提前写“已通过”、覆盖未确认分支或修改 8100。发现 SHA、影响指纹、状态、制品或 8100 隔离异常时，同一项目负责人必须立即停止流程。
...
4. 当前发布包原文件不可定位，仅在 verified state 中保留 SHA-256。
5. 本轮按用户明确指令没有运行任何新增数据、报告或 Word 门禁。
