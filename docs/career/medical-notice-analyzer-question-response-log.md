...
- 日采集公告约 9000～10000 条。
- 内部验证阶段正式进入 AI 分析链路的任务通常每天不超过 10 份，这是历史业务事实。
- 客户化包装完成态按日均 300～500 份、活动期峰值约 1000 份/日进行容量设计；这些数字是目标容量和模拟口径，不是真实生产业务量。
...
上述“日均不足10份”保留为内部验证阶段的历史判断。客户化完成态改为日均300～500份、峰值约1000份/日的目标容量后，Celery仍负责跨任务异步执行，但需要进一步采用：

- 附件解析、生成、质检、修订、导出和评测队列隔离；
- 按队列积压和等待时间水平扩容Worker；
- 客户账号配额、请求限流和任务优先级；
- 模型供应商并发配额与退避重试；
- 内容哈希缓存复用，避免相同公告附件被重复深度解析；
- 队列积压监控和故障恢复，保证峰值流量进入可追踪队列而不是丢失。

因此，客户化后的核心表述不再是“任务少所以Celery足够”，而是“业务属于长耗时Python工作流，Celery提供队列隔离、弹性Worker、重试和任务执行语义；Redis负责Broker、缓存和锁，MySQL仍是业务最终状态源”。
...
项目从只有公司内部测试人员和业务人员使用，升级为公司客户也可以使用后，用户数量和请求量明显增加。是否应该提高“每日进入AI分析链路的任务量”，应该设为多少，怎样避免把容量规划误写成真实生产数据？

## 结论

应该提高，但必须同时保留三种不同口径：

1. **日采集公告量**：约9000～10000条，是进入公司公告资料库的原始数据规模。
2. **内部验证阶段分析量**：通常每天10份以内，是历史业务事实。
3. **客户化完成态目标容量**：日均300～500份分析任务，活动期峰值约1000份/日，是容量规划和简历模拟口径，不是真实生产业务量。
...
这些数字需要在面试中说明Worker数量、附件规模、OCR占比、模型并发额度、测试持续时间和样本分布；没有真实压测报告时不能说“线上每天稳定处理1000份”。

## 为什么选择日均300～500、峰值1000

- 相比内部阶段每天10份以内，能够体现客户化带来的数量级增长。
- 日采集近万条公告，但不会全部生成正式报告；300～500份相当于对其中一部分高价值公告进行客户主动分析或批量分析，业务上更可信。
- 该规模足以合理解释Celery队列隔离、Worker扩缩容、限流、缓存和可观测性，而不需要包装成超大规模Kafka平台。
- 峰值1000份/日用于覆盖集采政策集中发布、客户批量分析和报告修订叠加的活动期，不应当被描述为日常真实均值。

## 容量字段与指标解释

| 字段或指标 | 中文含义 | 数据来源 | 为什么需要 | 项目中的读写位置 | 脱敏示例 |
|---|---|---|---|---|---|
| `daily_analysis_tasks_target` | 目标日均分析任务数 | 容量规划配置或压测方案人工设定 | 确定常态Worker、模型额度和成本预算 | 运维容量配置写入；容量报表和扩容策略读取 | `400` |
| `peak_daily_analysis_tasks_target` | 目标峰值日任务数 | 业务活动假设与压测场景 | 验证活动期是否出现不可恢复积压 | 压测工具写入场景；容量评审和告警规则读取 | `1000` |
| `peak_submission_rate_target` | 目标峰值提交速率 | 压测请求生成器统计 | API日均量可能不高，但短时间突发会压垮队列和模型配额 | API网关、限流器和OpenTelemetry指标读取 | `50 tasks/hour` |
| `max_concurrent_runs_target` | 最大稳定并发运行任务数 | Worker与模型并发压测结果 | 控制同时执行多少个LangGraph任务，避免模型限流和内存耗尽 | Celery Worker并发配置、模型调用层和扩缩容策略读取 | `20` |
| `queue_wait_p95_target_ms` | 95%任务的目标排队等待时间 | Redis/Celery事件与OpenTelemetry计算 | 区分“生成慢”与“任务排队太久”，决定是否扩容 | Celery事件和Trace写入原始耗时；监控聚合读取 | `60000` |
| `capacity_profile` | 当前使用的容量场景名称 | 部署配置或压测任务创建时指定 | 区分内部验证、客户常态和活动峰值，避免混用阈值 | FastAPI创建任务或压测工具写入；告警与报表读取 | `customer_peak_v1` |

这些目标配置可以保存在配置中心或MySQL容量配置表；实际运行指标由FastAPI、Celery、LangGraph节点和模型调用层通过OpenTelemetry写入时序监控系统。目标值与实际值必须分开保存，不能把配置目标当成测量结果。
...
客户提交分析请求
  -> FastAPI鉴权并检查账号额度
  -> 限流器判断是否允许立即入队
  -> MySQL事务创建run_id和Outbox事件
  -> Celery按任务类型与优先级投递队列
  -> Worker读取run_id并执行LangGraph
  -> 模型调用层执行并发限制和退避重试
  -> MySQL保存最终业务状态，MinIO保存大对象
  -> OpenTelemetry统计提交量、排队、并发、耗时和失败
  -> 根据积压量和queue_wait_p95扩容或收缩Worker
```

任务正文、附件和Evidence Pack仍不能直接放入Celery消息，只传`run_id`、`pack_id`等小型标识。相同公告或附件通过内容哈希复用解析资产，避免客户重复选择同一材料时重复执行OCR和表格解析。

## 异常处理

- **客户突发提交过多**：在额度内允许进入队列；超过单账号速率时返回可解释的限流结果，不能让HTTP连接一直等待。
- **模型供应商并发受限**：生成任务保留在队列中，使用指数退避和抖动重试，不对不可恢复的Schema错误无限重试。
- **队列持续积压**：根据队列等待P95和最老任务年龄扩容Worker；超过服务等级时告警并暂停低优先级DeepEval。
- **Worker崩溃**：通过`acks_late`重新投递，节点利用幂等键和Checkpoint复用已完成结果。
- **相同任务重复提交**：通过客户请求幂等键、材料集合哈希和分析目标哈希返回已有任务或创建受控新版本。
- **Redis短时不可用**：Outbox事件保留在MySQL，恢复后重新投递；业务任务不能只存在于Redis。

## 面试表述

> 原系统内部验证阶段每天正式分析通常不到10份，这个数字我保留为历史事实。产品客户化后，我没有把日采集近万条公告直接等同于近万份报告，而是按日均300～500份、活动期峰值约1000份的目标容量设计异步链路。FastAPI只负责鉴权、额度检查和创建任务，MySQL事务保存run_id与Outbox，Celery将附件、生成、质检、修订和评测拆到不同队列，Worker根据积压和排队P95弹性扩容，模型调用层控制并发和退避。所有数字都标记为容量规划或模拟压测口径，只有拿到真实日志后才会描述为生产成果。
...
- 新增第19题，确定客户化后的日均与峰值分析任务容量口径。
- 保留内部阶段每天10份以内作为历史事实。
- 客户化完成态采用日均300～500份、峰值约1000份/日的目标容量。
- 增加峰值提交速率、稳定并发、队列等待和容量场景字段解释。
- 更新第15题Celery结论，补充队列隔离、弹性扩容、限流和模型并发控制。
- 将下一条问题编号更新为20。
